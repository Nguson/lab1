    ...
    public Triangle(double aIn, double bIn, double cIn) {
        ...
        if (aIn <= 0 || bIn <= 0 || cIn <= 0) {
            throw new IllegalArgumentException("Sides: " + aIn + " " + bIn + " " + cIn
                    + " -- Each side cannot be less than or equal to 0");       // add an explanation why this exception is being thrown.
        }

        if ((aIn >= bIn + cIn) || (bIn >= aIn + cIn) || (cIn >= aIn + bIn)) {
            throw new IllegalArgumentException("Sides: "
                    + aIn + " " + bIn + " " + cIn
                    + " -- Sum of 2 sides cannot be greater than a side");      // add an explanation why this exception is being thrown.
        }
